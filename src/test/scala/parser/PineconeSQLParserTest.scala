...
class PineconeSQLParserTest extends AnyFlatSpec {
	val expectedResultOfDateFunc = s"SELECT * FROM WHERE DATA_DATE = ${getDateObject(1)}"

//	"A SQL Query" should "VEQV" in {
//		val result = PineconeSQLProcessor("SELECT * FROM WHERE DATA_DATE = ${=get_date(1)}")
...
//	}
//
//	it should "work well if no symbol identified" in {
//
//	}
}
